...
SEXP padFun(SEXP x, SEXP dates) {
  ...
  TSeries<TDATE,TDATA,TSDIM,TSDATABACKEND,DatePolicy> ans = ts.template pad(R_allocator<TDATE>::R_dataPtr(dates),R_allocator<TDATE>::R_dataPtr(dates) + length(dates));
